...
public class BookAction extends ActionSupport{
	...
	private String question1,question2,question3,question4,question5,question6;
	...
	private void userEstablishList(String user,String list)
	{
		...
		PreparedStatement searchUserPreparedStatement=tempConnection.prepareStatement(searchUserString);
		PreparedStatement updatePreparedStatement=tempConnection.prepareStatement(updateString);
		searchUserPreparedStatement.setString(1, user);
		ResultSet tempSet=searchUserPreparedStatement.executeQuery();
		if(tempSet.next())
		{
			StringBuilder tempString=new StringBuilder(tempSet.getString("questionlists"));
	...
	public String establishMyList()
	{
		...
		Connection tempConnection=bookConnection.getConnection();
		PreparedStatement searchstmt=tempConnection.prepareStatement(searchString);
		...
		{	
			...
			for(int i=0;i<list.size()&&i<answer.size();i++)
			{
				...
				System.out.println("i="+i);
				if(questionString.length()!=0&&answerString.length()!=0)
				{
					System.out.println("i="+i);
					//PreparedStatement searchQuestionstmt=tempConnection.prepareStatement(searchQuestionString);
					//searchQuestionstmt.setString(1, tempString);
					//ResultSet questionResultSet=searchQuestionstmt.executeQuery();
					...
					//}
					//else {
						PreparedStatement addQuestionstmt=tempConnection.prepareStatement(addQuestionString);
	...
	public String signup()
	{
		...
		try{
			...
			{	
				...
				PreparedStatement signupstmt=tempConnection.prepareStatement(signupString);
	...
	public String login()
	{
		try{
			...
			Connection tempConnection=bookConnection.getConnection();
			PreparedStatement loginstmt=tempConnection.prepareStatement(loginString);
